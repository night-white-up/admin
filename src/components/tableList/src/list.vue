 ...
 * @LastEditTime: 2021-11-25 14:58:37
 -->
<template>
      ...
      <el-table-column :type="expand ? 'expand' : null" v-if="expand">
        <template slot-scope="props">
          <el-table
            ...
            class="expand-table"
            :border="false"
            :row-class-name="childrentableRowClassName"
          >
            <!-- 第二层展开 -->
            <el-table-column type="expand" v-if="childrenExpand">
              <template slot-scope="scope">
                <el-table
                  ...
                  :border="false"
                >
                  <template v-for="(value, key, index) in childrenExpand">
                    <el-table-column
                      :prop="key"
                      :label="value.label"
                      ...
                      :type="value.type"
                      :sortable="
                        value.sortable != 'undefined' ? value.sortable : false
                      "
                      :align="value.align"
                    >
                    </el-table-column>
                      ...
                      <template slot-scope="scope">
                        <template v-for="(itemkey, indx) in value.condition">
                          <span
                            :key="indx"
                            :style="itemkey.style"
                            v-if="scope.row[key] === itemkey.value"
                            >{{ itemkey.label }}</span
                          >
                        </template>
                      </template>
                    ...
                    <el-table-column
                      ...
                      :align="value.align"
                    >
                      <template slot-scope="scopechildren">
                        <template v-for="(opr, index) in value.prop">
                          ...
                          <el-button
                            ...
                            v-if="
                              opr.change &&
                                opr.change.value.indexOf(
                                  scopechildren.row[opr.change.propName]
                                ) != -1
                            "
                            >{{ opr.name }}</el-button
                          >
                ...
                <template slot-scope="scope">
                  <template v-for="(opr, index) in value.prop">
                    <el-button
                      :icon="opr.icon"
                      :key="index"
                      :class="opr.class ? opr.class : null"
          ...
          <template slot-scope="scope">
            ...
            >
              ...
              <span
                ...
                class="cnt"
                :style="{
                  textDecoration: item.underline ? 'underline' : null,
                  color: item.color,
                }"
                >{{ scope.row }}</span
              >
          ...
          <template slot-scope="scope">
            ...
            <template v-for="(key, index) in item.prop">
              <el-button
                ...
                v-if="
                  scope.row[
                    item.change ? item.propName + '1' : item.propName
                  ] === key.value
                "
                :icon="key.icon"
              ...
              >
                <span v-if="key.after" v-html="key.after"></span
                >{{
                  key.hasOwnProperty("addjudegment")
                    ? scope.row[key.addjudegment.prop].includes(
                        key.addjudegment.value
                      )
                      ? key.label
                      : null
                    : key.label
            ...
            <template v-for="(opr, index) in item.prop">
              <template v-if="opr.params">
                <el-button
                  ...
                  :style="{ color: opr.color, fontSize: opr.fontSize }"
                  v-if="
                    (String(opr.params.value).indexOf(
                      String(scope.row[opr.params.prop])
                    ) > -1 ||
                      String(scope.row[opr.params.prop]).indexOf(
                        String(opr.params.value)
                      ) > -1) &&
                      !opr.params.show
                  "
                  :type="!opr.type ? 'success' : opr.type"
              ...
              >
                <template v-for="(key1, idx) in opr.condition">
                  <span v-if="key1.equality" :key="idx">{{
                    scope.row[opr.prop] ? key1.label : null
                  }}</span>
                  <span
                    :key="idx"
                    :style="
                      key1.value ===
                      scope.row[opr.prop ? opr.prop : opr.prop + '1']
                        ? key1.style
                        : null
                    "
                    v-else-if="!key1.equality"
                  >
                    {{
                      key1.value ==
                      scope.row[opr.prop ? opr.prop : opr.prop + "1"]
...
.rownone .el-table__expand-icon {
  display: none !important;
}
.rowchildren .el-button {
  display: none;
}
.el-button--text {
