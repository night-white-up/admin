...
<template>
	<div class="form-box">
		<el-form :inline="true"  v-if="defultDataFrom" :model="formInline" class="el-list-query"  :label-width="formInline.labelWth"  ref="formInline">
			<el-form-item v-for="(item,index) in defultDataFrom" :key="index" :label="item.itemName">
				<el-select
				...
				></el-input>
					<!-- 文本 -->
				<el-input type="textarea" v-model="formInline.paramList[item.model]" v-if="item.type=='texterea' &&item.show!=false" :size="item.size==undefined?'medium ':item.size"
					:placeholder="item.placeholder"
					:prop="formInline.paramList[item.model]"></el-input>
				<el-date-picker
				...
				<el-autocomplete
					...
					@select="(value)=>{formInline.paramList[item.model] = value.value}"
					:prop="formInline.paramList[item.model]"
				>
		...
		<el-form>
			<el-form-item>
				<el-button type="primary"  @click="onSubmit" v-if="formInline.show !='rename'">查询</el-button>
				<el-button type="primary"  @click="onSubmit" v-else >{{formInline.name}}</el-button>
				<template  v-for="(item,index) in formInline.btn">
						<el-button  :key="index"  v-if="item.type=='reset'"  @click="resetForm(item.click)" :disabled="item.disabled" >{{item.name}}</el-button>
						<el-button  :key="index"  v-else  @click="item.click" :disabled="item.disabled" >{{item.name}}</el-button>
...
import LongSearch from "./components/long_randge_search/Index"
export default {
	name: "listQuery",
	data() {
		return {
